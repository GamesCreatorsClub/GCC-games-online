...
screensize = width, height = 1024, 512
...
def moveMrbounce():
    ...
    sx = x
    sy = y
    if mrbouncerect.colliderect(paddleRect) or mrbouncerect.colliderect(paddle2Rect):#(sx + mrbouncerect[2] > 1024 - paddle2Rect[2] and sy > paddle2Rect[1] and sy < paddle2Rect[1] + paddle2Rect[3]) or (sx < 0 + paddleRect[2] and sy > paddleRect[1] and sy < paddleRect[1] + paddleRect[3]):
        if not hitPaddle:
