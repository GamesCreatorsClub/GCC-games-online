...
class Player(pygame.sprite.Sprite):
    ...
    def get_interact(self,level, dt, side,keys):
        ...
        if (pygame.KEYDOWN,pygame.K_r) in level.keyed:
            if self.equipped:
                if self.equipped.mag < self.equipped.mag_size and self.ammo[self.equipped.ammo_type][0] > 0:
    ...
    def update(self,level, dt):
        ...
        if not self.unloaded:
            ...
            if self.selected:
                if self.equipped:
                    if side == 'left':
                        self.equipped.update(level.projectiles, dt, self.rect.midleft,level,self.ammo)
                        self.equipped.flip = True
                    else:
                        self.equipped.update(level.projectiles, dt, self.rect.midright,level,self.ammo)
