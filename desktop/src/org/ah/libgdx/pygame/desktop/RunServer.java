...
public class RunServer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        File gamesDir = new File(new File("").getAbsoluteFile().getParentFile(), "games");
    ...
    private static void processFile(PrintWriter a, String prefix, File dir) {
        String p = "";
        if (!prefix.equals("")) {
            a.println("d:" + prefix + ":" + prefix + ":0:application/unknown:1");
            p = prefix + "/";
        }
        for (File f : dir.listFiles()) {
            if (!f.getName().startsWith(".")) {
                if (f.isDirectory()) {
                    ...
                    a.println(t + ":" + p + n + ":" + p + n + ":" + f.length() + ":" + mimeType(n) + ":" + 1);
                }
            }
        }
    }
